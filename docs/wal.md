...
On database-format-v2 active pages, `full-page` emits type 9, `byte-range` emits type 10,
and `adaptive` compares their complete encoded sizes per update. Unsupported/legacy page
formats use the corresponding legacy record when necessary. Recovery remains driven by
the persisted record type. Exact v2 payloads and the normalization rule are in
...
## Persistent and volatile PageLSN enforcement

Each `BufferFrame` has volatile `pageLsn`. `WritePageGuard::setPageLsn()` requires a write pin,
...
are rejected. Active database page formats also store the value in an explicit
little-endian field. The coordinator assigns the record LSN to both representations
after append; reload reconstructs the volatile state from persistent bytes.
